...
void XmlWriter::writeEndElementNULLTag() {
	if (outFile.is_open()) {
		outFile << " />\n";
		tempElementTag.resize(openElements - 1);
		openElements -= 1;
...
void XmlWriter::writeEndElementBox() {
	if (outFile.is_open()) {
		outFile << " />";
		tempElementTag.resize(openElements - 1);
		openElements -= 1;
...
void XmlWriter::writeCloseNewTag() {
	if (outFile.is_open()) {
		outFile << ">\n";
	}
	else {
...
void XmlWriter::writeStartElementNewTag(const std::string elementTag) {
	if (outFile.is_open()) {
		for (int i = 0; i < indent; i++) {
			outFile << "\t";
		}
...
void XmlWriter::writeStartElementBox(const std::string elementTag) {
	if (outFile.is_open()) {
		outFile << ">\n";
		for (int i = 0; i < indent; i++) {
			outFile << "\t";
